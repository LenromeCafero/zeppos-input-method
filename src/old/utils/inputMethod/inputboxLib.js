...
class TextLine {
    constructor({ text, border, text_size, color, beginSafetyDistance }) {
        ...
        this.leftWidth = this.border.w - dataManager.json.safetyDistance;
        this.textShow = text;
    ...
    setLocX(locX) {
        ...
        if (
            this.charEnd == this.textLength - 1 ||
            tempX - locX >= this.border.w
        ) {
        } else {
            for (
                ++this.charEnd;
                this.charEnd < this.textLength;
                ++this.charEnd
            ) {
                tempX += this.textWList[this.charEnd];
...
export const InputBoxLib = [
    class NORMAL {
        constructor({ text, father, title }) {
            ...
            this.safetyDistance = dataManager.json.safetyDistance; // 删除和输入时光标距离边框的最大水平距离
        ...
        onCreate() {
            if (dataManager.json.theme.inputbox.background.type == "img") {
                this.backgroundWidget = hmUI.createWidget(hmUI.widget.IMG, {
                    x: 0,
                    y: 0,
                    src: dataManager.json.theme.inputbox.background.src,
                });
            } else if (
            ...
            logger.debug(dataManager.json.theme.inputbox.inputBoxMask.src);
