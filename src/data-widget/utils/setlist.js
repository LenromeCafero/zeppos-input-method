...
export class SetTool {
  ...
  insertPage(page, index) {
    if(!page) {return -1}
    if((!index && index != 0) || index >= this.pageArray.length) 
...
export class SetPage {
  ...
  show() { //可以考虑加个index来局部重绘制
    ...
    this.pageHeight = 0 //作为当前item的y坐标偏移
  ...
  insertItem(item, index) {
    if(!item) {return -1}
    if((!index && index != 0) || index >= this.itemArray.length) 
  ...
  show(offsetY) {
    ...
    switch(this.style) {
      ...
      case SetItemMaker.Styles.BODY: {
          ...
          //TEXT大文字
          if(text) {
            let textWidth = px(260) - ((icon)?this.data.icon.w + px(10):0)
    ...
    //console.log("fuck 3")
          ...
          if(this.type & SetItemMaker.Types.ARROW) { //箭头跳转
            this.widgets["arrow"] = hmUI.createWidget(hmUI.widget.BUTTON, {
              ...
              h: px(100),
              text: '',
              press_src: 'cuberSet/enter.png',
              normal_src: 'cuberSet/enter.png',
              click_func:() => {
            ...
            this.widgets["switch"] = hmUI.createWidget(hmUI.widget.SLIDE_SWITCH, {
              x: px(350),
              y: (height - px(64)) / 2 + offsetY + 5,
              w: px(100),
              h: px(64),
              un_select_bg: "cuberSet/false.png",
              select_bg: "cuberSet/true.png",
              slide_src: "cuberSet/button.png",
            ...
            let radioGroup = hmUI.createWidget(hmUI.widget.RADIO_GROUP, { 
              ...
              select_src: "cuberSet/radioSelect.png",
              unselect_src: "cuberSet/radioUnselect.png",
          ...
          //SUBTEXT小文字
          if(subtext) {
            let {height: subtextH, width: subtextW} = 
            hmUI.getTextLayout(this.data.subtext.text, {
              text_size: px(30),
